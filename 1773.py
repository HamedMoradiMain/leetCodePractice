...
the ith item. You are also given a rule 
...
class Solution:
    def countMatches(self, items: list[list[str]], ruleKey: str, ruleValue: str) -> int:
        res = 0
        if ruleKey.lower() == "type":
            counter = 0
        if ruleKey.lower() == "color":
            counter = 1
        if ruleKey.lower() == "name":
            counter = 2
        for item in items:
            if item[counter] == ruleValue:
                res += 1
        return res
sol = Solution()
print(sol.countMatches(items = [["phone","blue","pixel"],["computer","silver","lenovo"],["phone","gold","iphone"]], 
                 ruleKey = "type", 
                 ruleValue = "phone"))